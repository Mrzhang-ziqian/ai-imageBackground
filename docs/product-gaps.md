...
| G23a | **页面逻辑 & 登录策略缺陷 (G23 增强)** | ① 点「重新上传」后历史面板消失（v-if 切换 Bug）；② 匿名用户无限白嫖（无痕窗口绕过配额）；③ 相同照片反复处理产生重复历史记录。后端配额无日重置机制，用完永久卡死 | 🔜 设计已完成 (`docs/design-logic.md`)，待实施 |
...
### 🎯 当前焦点：Phase 5 — 商业化基础（页面逻辑增强）

> Phase 1~4 全部交付。G23 用户体系已上线，但发现 3 个页面逻辑问题 + 匿名白嫖漏洞。
> **当前最高优先：完成 `docs/design-logic.md` 中的页面逻辑重构，然后推进 G24 付费墙。**

| 优先级 | ID | 任务 | 工作量 | 理由 |
|:---:|:---:|------|:--:|------|
| 🥇 | **G23a** | **页面逻辑 & 登录策略重构** | 2 天 | 修复历史可见性 Bug + 匿名试用 2 次 + IP 限频 + 配额日重置 + 历史去重。详见 `docs/design-logic.md` |
...
> **下一步**：立即实施页面逻辑 & 登录策略重构（G23a），然后推进 G24 付费墙。
