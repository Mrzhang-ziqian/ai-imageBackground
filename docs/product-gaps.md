...
> 更新时间：2026-05-23  
> 版本：v4.8  
> 版本说明：Phase 1~4 核心层全部完成。Sprint A~L 共 37 项 UX 优化 + 86 项审计热修复。G41~G43 全部完成。Sprint L 修复 G43 P1×7 + P2×15。
...
| G24 | **零变现设计** | ✅ 已实现 (Phase 5) — Stripe 支付集成 + Freemium 付费墙 + Pro 功能门控 |
...
Sprint K: 🧪 G43 全代码库审计 (58 项发现)
Sprint L: ████████████ 22/22 G43 P1×7 + P2×15 修复 (全部完成)
──────────────────────────────────────
合计:     60 项UX优化 + 86 项审计热修复
剩余:     G43 P3 30 项 + G43 K8 + Canvas缩放 (P2)
```
...
### 🛠 Sprint L — G43 P1 + P2 修复（2026-05-23）

| # | 问题 | 修复方式 |
|:---:|------|------|
| **K1** | BatchPanel `retryingIds` Set 响应式失效 | `ref<Record<string, boolean>>({})` 替代 `ref<Set<string>>(new Set())` |
| **K2** | BatchPanel `resultUrls` Map 响应式失效 | `ref<Record<string, string>>({})` 替代 `ref<Map<string, string>>(new Map())` |
| **K3** | useAuth 初始化竞态条件 | 新增 `initialized` ref，fetchMe 完成后设为 true |
| **K4** | api.ts getMe/authFetch 逻辑重复 | `getMe` 复用 `authFetch` 统一错误处理 |
| **K5** | XHR 读 localStorage 绕过 Store | `uploadAndRemoveBg` 新增 `authToken` 参数，调用方传入 |
| **K6** | DraftDetailPage handleConfirm 回滚缺失 | 先 `router.replace` 再 `drafts.remove`，防止导航失败丢数据 |
| **K7** | handleConfirm 先 updateResult 再 remove | 跳过 `updateResult`（即将删除的草稿无需保存） |
| **K9** | 数据库种子密码硬编码 | 启动时检查环境变量，使用默认密码时输出 WARNING |
| **K10** | migration 每次执行失败的 ALTER TABLE | 先 `PRAGMA table_info` 检查列是否存在再执行 |
| **K11** | DATABASE_URL 相对路径依赖 CWD | 基于 `__file__` 计算绝对路径 |
| **K12** | useHistory remove/clearAll 静默吞错误 | 乐观更新改为 API 成功后更新 UI |
| **K13** | useHistory reload 与 load 功能重复 | `reload` 改为 `load` 的别名 |
| **K14** | useQuota 加载期间显示错误默认值 5 | 未登录时 `quotaDaily` 返回 `null` 让 UI 显示加载态 |
| **K16** | useAuth fetchMe 非 401/403 错误静默 | 设置 `error` 状态让 UI 可重试 |
| **K17** | UploadZone 全局 dragover 阻止默认行为 | 移除全局事件监听，仅 dropzone 内处理 |
| **K18** | HistoryPanel blocked 条目 shimmer 永不止 | blocked 条目设 `resultLoading: false` |
| **K19** | BackgroundColorPicker 死 CSS 样式残留 | 删除无对应元素的 `.color-picker-panel` 等规则 |
| **K26** | 登录接口密码 min_length=1 与注册不一致 | 改为 `min_length=8` |
| **K28** | XHR error 事件中 abort 标志竞态 | 保留 `signal.aborted` 检查（已有 set flag 逻辑） |
| **K29** | onDownloadSingle 每次创建新 ObjectURL | 复用 `getResultUrl` 缓存 |
| **K30** | POST 请求使用 Date.now() cache-busting | 移除 `?_t=${Date.now()}` |
| **K31** | MAX_HISTORY 导入未使用 | 删除未使用导入 |

> **K8** (composables → Pinia) 涉及架构重构，留待后续。

### 📊 G43 审计结论

```
┌─────────────────────────────────────────────────────────────┐
│  Sprint K+L (G43) 全代码库审计 + 修复                         │
│                                                             │
│  审计范围:  13 个文件（后端 3 + 前端 10）                     │
│  发现问题:  58 项（8 P1 + 23 P2 + 27 P3）                   │
│  已修复:    22 项（7 P1 + 15 P2）                           │
│  致命 BUG:  0 项                                            │
│  剩余:      K8 (P1 架构) + 30 P3 + Canvas缩放 (P2)           │
└─────────────────────────────────────────────────────────────┘
```
