...
> 更新时间：2026-05-22  
> 版本：v3.1  
> 版本说明：Phase 1~4 核心层全部完成。Phase 5 G23 用户体系已上线，但发现3个页面逻辑问题 + 匿名白嫖漏洞，已完成设计方案（`docs/design-logic.md`），待实施。
...
✅ 新增              G22 大图保护            1/1
🔧 Phase 5 (壁垒)   G23 用户体系 ✅ → 待增强  1/4  (设计完成)
...
| 优先级 | # | 问题 | 影响 | 对应报告 |
|:---:|:---:|------|------|:---:|
| 🔴 | 16 | **DraftDetailPage Object URL 泄漏** | 每次进入/离开草稿详情页泄漏 2 个 Blob URL | P0-1 |
| 🔴 | 17 | **LandingPage RAF 动画未清理** | 组件销毁后 RAF 回调持续执行，操作已销毁 ref | P0-2 |
| 🔴 | 18 | **HistoryPanel thumbStates 永久积累** | 历史条目删除后 key 不清理，内存缓慢泄漏 | P0-3 |
| 🔴 | 19 | **配额 TOCTOU 竞态条件** | 两个并发请求可同时通过检查，quota_used > quota_daily | P0-4 |
| 🔴 | 20 | **JWT 默认弱密钥** | 未设环境变量时使用可预测密钥，token 可被伪造 | P0-5 |
| 🟠 | 21 | **Pro 按钮 pointer-events 被拦截** | `.pro-card` 覆盖按钮，用户无法点击「了解详情」 | P1-1 |
| 🟠 | 22 | **缩放失败静默使用大图** | 用户选择缩放但失败后悄悄处理原图，绕过用户意图 | P1-2 |
| 🟠 | 23 | **草稿原图丢失无提示** | IndexedDB 缺少原图时显示两张相同图片，对比功能失效 | P1-3 |
| 🟠 | 24 | **无文件魔数校验** | 可伪造 Content-Type 绕过类型检查上传恶意文件 | P1-6 |
| 🟠 | 25 | **无速率限制** | 登录/注册/上传无频率限制，可被暴力破解和刷爆存储 | P1-9 |
| 🟠 | 26 | **gc.collect() 阻塞事件循环** | 同步阻塞操作在 async 函数中，影响并发请求 | P1-10 |
| 🟠 | 27 | **大文件先读入内存才检查大小** | 攻击者可上传超限文件耗尽内存 | P1-11 |
| 🟡 | 28 | **PreviewGrid 背景色硬编码** | 用户选非透明背景时预览盒始终显示灰色 | P2-5 |
| 🟡 | 29 | **constructor.name 压缩后失效** | Terser 压缩使 errorHumanizer 检查永远不命中 | P2-7 |
| 🟡 | 30 | **Docker backend 直接暴露 8000** | 绕过 nginx 代理，直连后端 | D-1 |
| 🟡 | 31 | **Docker data 目录未挂载** | 容器重建后 SQLite 用户数据丢失 | D-4 |
| 🟡 | 32 | **异常处理器泄露内部信息** | 500 错误返回原始异常消息，可能泄露路径/版本 | P1-7 |
...
> **当前状态**：Phase 1~4 全部交付（G01~G09+G12+G14+G22 共 12 项）。G23 用户体系已上线，发现 3 个页面逻辑问题 + 匿名白嫖漏洞，设计方案已完成（`docs/design-logic.md`）。
> **下一步**：立即实施强制登录重构（G23a），废弃匿名试用，然后推进 G24 付费墙。
