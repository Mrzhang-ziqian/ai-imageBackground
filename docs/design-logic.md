...
### 13.1 问题 A：单图查看后无法返回多图浏览（当前会话上下文缺失）

#### 现象
用户同时上传 5 张图片处理后，点击其中某一张进入 DONE 视图 → 无法返回查看其他已处理的图片。只能刷新页面让历史面板重新加载。

#### 根因
当前的交互模型是 **"模态单图编辑器"**：PreviewGrid 进入 DONE 状态后完全替换为单张大图 + 工具栏，没有「返回全部」或「切换查看」的入口。`handleHistoryRestore()` 可以恢复历史条目，但这绕了一大圈——用户上传完的图片应该还在"当前会话"中直接可访问。

#### 设计方案：底部胶片条（Bottom Filmstrip）
...
| 决策点 | 选择 | 理由 |
|--------|------|------|
| 交互模式 | 底部胶片条（始终可见） | 零模式切换成本，一键切换 |
| vs 返回按钮 | ❌ 不选 | 多一次点击，效率低 |
| vs 全屏网格 | ❌ 不选 | 每次退出编辑模式体验差 |
| 数据来源 | **前端内存状态**（非数据库） | "当前会话"是临时的，刷新即消失。持久化数据在 HistoryPanel 已有 |
| 切换方式 | 点击缩略图 + 键盘 ← → | 适配鼠标和键盘用户 |
| 可见条件 | DONE 状态 + 当前会话图片 ≥ 2 张 | 只有一张时不显示胶片条避免冗余 |
| 选中态 | 蓝色边框 + 发光 + 底部蓝色指示条 | 与品牌色一致，一目了然 |
| Pro 彩蛋 | 胶片条多选 → 左右分屏对比 | 作为 Pro 卖点预留 |

#### "当前会话" vs "历史记录"

```
当前会话（SessionInMemory）
  ├── 存储位置：前端内存（RemoverState 或独立 ref）
  ├── 生命周期：本次浏览器标签页存活期间
  ├── 数据内容：本次上传批次处理的所有结果
  ├── 清空条件：handleReset() / 标签页关闭
  └── 作用：提供胶片条的图片列表
...
**两者互补**：胶片条用于"当下快速切换"，历史面板用于"长期查找恢复"。

---

### 13.2 问题 B：「重新上传」缺少确认 + 配额感知

#### 现象
1. 点击「重新上传」按钮无任何确认，直接清空已处理结果 — 误触风险高
2. 不清除用户当前配额使用情况 — 用户不知道可以重新上传多少张
3. 配额已耗尽时仍然显示「重新上传」— 浪费操作

#### 设计方案：两级弹窗（按剩余配额分流）

##### 情况 A：还有剩余额度（例如用了 2/5）
...
- 明确告知「剩余可上传 3 次」建立预期
- 文案柔和：「清空当前结果」而非「删除」
- 「取消」为次要按钮（灰色副色调），「确认重新上传」为主要按钮（品牌色）
...
**设计要点：**
- 左按钮「明天再说」带幽默感，降低挫败感
- 右按钮「升级 Pro」金色渐变，引导转化
- 列出 3 个 Pro 核心卖点，不啰嗦
- Pro 按钮点击后当前阶段弹 toast，后续对接 Stripe

##### 情况 C：Pro 用户
不弹确认窗，直接清空。Pro 用户无限配额，不需要确认。

##### 配额计算逻辑总结

| 用户类型 | 已上传 | 剩余 | 点击「重新上传」 | 弹窗类型 | 清空后可再传 |
|---------|--------|------|----------------|----------|:---:|
| Free    | `N < 5` | `5 - N` | → | 情况 A（确认弹窗） | `5 - N` 条 |
| Free    | `5` | `0` | → | 情况 B（升级引导） | 0（引导升级） |
| Pro     | 任意 | ∞ | → | 无弹窗 | 无限 |
...
#### 现象
右上角头部显示 "剩余 NaN 次"，用户认为产品出了 bug。

#### 根因
`useAuth.ts` 和 `useQuota.ts` 中：
```typescript
const quotaLeft = computed(() => {
  return Math.max(0, user.value.quota_daily - user.value.quota_used)
})
```
当 `/auth/me` 返回的 `quota_daily` 或 `quota_used` 为 `undefined/null` 时：
- `undefined - undefined` → `NaN`
- `Math.max(0, NaN)` → `NaN`
- 模板渲染 `"剩余 NaN 次"`

#### 修复方向
1. **前端防御**：`(quota_daily ?? 5) - (quota_used ?? 0)`，杜绝 NaN
2. **后端兜底**：确保 `/auth/me` 始终返回完整的 `quota_daily` 和 `quota_used` 字段
3. **加载态**：配额数据加载中时用骨架屏/`--` 代替数字，避免闪变
...
| 优先级 | 问题 | 影响 | 涉及文件 |
|--------|------|------|----------|
| 🔴 P0 | 问题 C：NaN 显示 | 产品看起来坏了 | `useAuth.ts`, `useQuota.ts`, `AppHeader.vue` |
| 🟠 P1 | 问题 B：重新上传确认 | 误操作 + 用户困惑 | `App.vue` + 新建 `ConfirmResetModal.vue` |
| 🟡 P2 | 问题 A：底部胶片条 | 核心体验缺失 | `App.vue` + 新建 `SessionFilmstrip.vue` |
...
| v2.0 | **废弃匿名试用 → 强制登录 + Landing Page + 新手引导 + Pro 占位 + 动画** | ✅ 已完成 |
| v2.1 | **底部胶片条 + 重新上传两级弹窗 + NaN 修复** | 🟡 设计中 |
...
> **核心理念 v2.1**：补好"回不去"和"误操作"两个体验窟窿。底部胶片条让多图浏览像翻书一样自然，两步弹窗让用户在做决定时信息透明。小而美的改进，体感提升巨大。  
